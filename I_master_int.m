...
if(~exist([DEMdir 'tmp.dem']))
    home=pwd;
    chdir(DEMdir);
    command=['get_SRTM.pl tmp.dem ' num2str([min([frames.lat])-1 max([frames.lat])+1 min([frames.lon])-1.5 max([frames.lon])+1.5 1 1])];
    mysys(command);
    chdir(home)
end
...
% fprintf(fid,['Alooks_sml=16\n']);
% fprintf(fid,['Alooks_unw=16\n']);
fprintf(fid,'\n');
