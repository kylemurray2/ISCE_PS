...
for l=1:length(rlooks)
  ...
  for k=1:nints     
    if(exist(ints(k).filtrlk,'file'))
      fidi=fopen(ints(k).filtrlk,'r');
      fido=fopen([ints(k).filtrlk 'tmp'],'w');
      for j=1:newny(l)
	...
	if(and(j>=id3,j<=id4))
      ...
      fclose(fido);
      fclose(fidi);
movefile([ints(k).filtrlk 'tmp'],ints(k).filtrlk);
    end
  end
end
