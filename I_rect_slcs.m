...
%  for i=1   
    ...
    if(~exist([offname '_fitoff.out']))
        command=['$INT_SCR/offset.pl ' dates(id).slc ' ' dates(i).slc ' ' offname ' 2 cpx ' num2str([dates(i).rgoff dates(i).azoff-20 noff noff offwin searchwin])]
        mysys(command);
...
%KM EDIT:  Make sure the rect file is not 0 size.  If it is, then delete
%all associated files and increase the search window size in set_params.m
% 
%             for i=[1:id-1 id+1:ndates]
...
%                     command=['rm ' rectdir '*' dates(i).name '*'];
%                     mysys(command)
%                     command=['rm ' rectdir 'rectfiles/*' dates(i).name '*'];
%                     mysys(command)
% %                     !sed -e "20s/.*/offwin=128;/" < set_params.m > set_params2.m;mv set_params2.m set_params.m
% %                     !sed -e "21s/.*/searchwin=128;/" < set_params.m > set_params2.m;mv set_params2.m set_params.m
% % 
%                     display([dates(i).name ' was bad and deleted. The window search offset was increased to 128 and the search window was increased to 128. Rerun this script'])
%                 end
%             end
%************************************
