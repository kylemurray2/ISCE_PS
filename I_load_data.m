...
switch sat
    ...
    case {'ALOS' , 'S1A'}
        ...
        command=['/usr/bin/wget --save-cookies cookies.txt --post-data=''user_name=' uname '&user_password=' passw ''' --no-check-certificate https://ursa.asfdaac.alaska.edu/cgi-bin/login -o /dev/null -O /dev/null'];
        system(command);
end
...
for i=1:length(searchresults)
    ...
    if(test.bytes<1e12) %some large number
        ...
        switch sat
            case {'ENVI','ERS'}
                command=['/usr/bin/wget -r -nv -c --http-passwd=' passw ' --http-user=' uname ' --directory-prefix=' dates(datei).dir ' -nd ' downloadpath];
                mysys(command);
            case {'ALOS','S1A'}
                command = ['/usr/bin/wget -c --http-passwd=' passw ' --http-user=' uname ' --no-check-certificate --load-cookies=cookies.txt --directory-prefix=' dates(datei).dir ' -nd ' downloadpath];                
                mysys(command);
